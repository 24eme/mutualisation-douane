#Accès aux fichiers XML via le serveur de mutualisation douane

Les fichiers reçu de la douane sont mis à disposition à travers une arborescence HTTP accessible aux partenaires du projet CNIV mutualisation Douane.
...
Pour avoir accès aux XML reçus des douanes, il faut réaliser une demande auprès de l'équipe vins d'Actualys.

## Accès aux XML

L'accès aux XML reçus de la plateforme ProDou@ne se faire via une arborescence qui répond au schéma suivant :
...
En interogeant les répertoires de l'arborescence, il est possible d'avoir un listing des XML qui s'y trouvent :
...
Ce mécanisme fonctionne pour tous les répertoires à partir de celui de présentant le SIREN de l'interpro :
...
Il est possible de passer deux parametres GET à ces requêtes de demande de listing des fichiers :

 - ``from`` afin d'indiquer à partir de quelle date on souhaite récupérer la liste des fichiers. La date doit être indiquée au format ISO 8601 (AAAA-MM-JJ)
 - ``format`` qui permet de demander une sortie dans un format XML

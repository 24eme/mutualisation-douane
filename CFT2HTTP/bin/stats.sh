...
join -t ';' -1 1 -2 2 /tmp/siren2interpro.csv /tmp/sirenmois.csv | awk -F ';' '{print $3";"$2";"$1";"$4}' | sort > publication_web/DRMInterpro.csv.tmp
mv -f publication_web/DRMInterpro.csv.tmp publication_web/DRMInterpro.csv
...
cat /tmp/xmlcrees.csv /tmp/archivesrecues.csv | sort > publication_web/DRMjour.csv.tmp
mv -f publication_web/DRMjour.csv.tmp publication_web/DRMjour.csv
